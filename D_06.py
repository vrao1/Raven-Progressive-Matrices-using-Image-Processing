...
def mainD06(problem):
	...
	sampleImageList.append(im1_shaded)
	...
	sampleImageList.append(im2_shaded)
	rect = (left, upper, right,lower)
	sampleImageRectangleList.append(rect)
	#im2_shaded.show()
	...
	sampleImageList.append(im3_shaded)
	rect = (left, upper, right,lower)
	sampleImageRectangleList.append(rect)
	#im3_shaded.show()
	...
	queryImageList.append(im7_shaded)
	#im7_shaded.show()
	...
	queryImageList.append(im8_shaded)
	#im8_shaded.show()
	...
	im9_contour = Image.new(im8.mode, im8.size, "white")
	im9_contour.putdata(im8_contour_arr)
	
	#im9_contour.show()
	
	lengthQuery = len(queryImageList)
	
	for i in range(lengthQuery):
		lengthSample = len(sampleImageList)
		for j in range(lengthSample):
			diff = rmsdiff_1997(queryImageList[i], sampleImageList[j])
			if diff == 0.0:
				del sampleImageList[j]
				del sampleImageRectangleList[j]
	
	
	im9_contour.paste(sampleImageList[0].crop(sampleImageRectangleList[0]), sampleImageRectangleList[0])
	#im9_contour.show()
